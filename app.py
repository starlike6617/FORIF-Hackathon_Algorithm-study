...
def compute_metric_scores(df: pd.DataFrame) -> pd.DataFrame:
    """편람 파일의 '%', 공백, _, (), 한글/영문 혼용 등 어지러운 열 이름을
       최대한 유연하게 매칭해 과제·조모임·성적 점수를 계산한다."""
    df = df.copy()

    # 1) 열 이름 정규화: 공백·밑줄·괄호·퍼센트 제거
    def _norm(s: str) -> str:
        return re.sub(r"[\s_()%]", "", str(s))

    # 2) {정규화된 이름: 원본 이름} 매핑 테이블
    norm2orig = {_norm(c): c for c in df.columns}

    # 3) 원하는 열을 찾는 헬퍼
    def _get(*aliases: str) -> pd.Series:
        for a in aliases:
            col = norm2orig.get(_norm(a))
            if col:
                df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)
                return df[col]
        # 매칭 실패 → 0으로 채운 가짜 열 생성
        df[aliases[0]] = 0
        return df[aliases[0]]

    # 4) 점수 계산 (가중치: 없음*2 + 보통*1)
    hw_none = _get("과제없음", "과제_없음")
    hw_mid  = _get("과제보통", "과제_보통")
    df["과제_score"] = (2*hw_none + 1*hw_mid) / 100

    team_none = _get("조모임없음", "조모임_없음")
    team_mid  = _get("조모임보통", "조모임_보통")
    df["조모임_score"] = (2*team_none + 1*team_mid) / 100

    grade_easy = _get("성적너그러움", "성적_너그러움")
    grade_mid  = _get("성적보통", "성적_보통")
    df["성적_score"] = (2*grade_easy + 1*grade_mid) / 100

    # 5) 화면 표시에 바로 쓸 수 있는 컬럼 생성
    df["과제 적음"]   = df["과제_score"].round(2)
    df["조모임 적음"] = df["조모임_score"].round(2)
    df["성적 후함"]   = df["성적_score"].round(2)
